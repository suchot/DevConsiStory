...
class ConsistoryXLPipeline(StableDiffusionXLPipeline):
    r"""
    Args:
    ConsistoryXL-Pipeline for text-to-image generation using Stable Diffusion. This model inherits from
    [`StableDiffusionXLPipeline`]. Check the superclass documentation for the generic methods the library implements for
    ...
    def show_cross_attention_list(self, prompts: list[str], res: int, from_where: List[str], select: int = 0, display: bool = False):
    ...
    def show_cross_attention_idx(self, prompts: list[str], res: int, from_where: List[str], select: int = 0, display: bool = False, token_id=1):
            tokens = self.tokenizer.encode(prompts[select])
            decoder = self.tokenizer.decode
            attention_maps = aggregate_attention(
                        self.controller,
            ...
            images = []
            tokens_idx = token_id
            heat_map_img = attention_maps[:, :, tokens_idx]
            heat_map_img = (heat_map_img - heat_map_img.min()) / (heat_map_img.max() - heat_map_img.min())
            img_otsu  = mask_with_otsu_pytorch(heat_map_img)
            return ToPILImage()(img_otsu)
    ...
    def __call__(
        ...
        attn_res = None,
        token_positions: Optional[int] = 76,
        concept_token: Optional[str]= 'EoT',
        dift_feature = None,
    ):
        r"""
        ...
        self.controller = AttentionConsiStore(attn_res=attn_res, token_positions=token_positions)

        self.register_attention_control(self.controller)  # add attention controller
        self.controller.dift_feature =  None
        ...
        feature_injection_start_timpstep = 680
        feature_injection_end_timpstep = 900
        with self.progress_bar(total=num_inference_steps) as progress_bar:                
            for i, t in enumerate(timesteps):
                if t > feature_injection_start_timpstep and t <= feature_injection_end_timpstep:
                    self.controller.dift_feature =  dift_feature
                else:
                    self.controller.dift_feature =  None
...
def main(detail, consistory_pipe, sdxl_pipe, dift, seed):
    prompts = detail['prompts'][:3] # big varm can afford large batch size.
    seed_everything(seed)
    g_cpu = torch.Generator().manual_seed(seed)
    base_image = sdxl_pipe(prompt=prompts, generator=g_cpu, height=1024, width=1024, guidance_scale=5, num_inference_steps=37)
    # for dift feature extract
    imglist = []
    ft = []
    image_size, ensemble_size = 512, 8

    for idx, img in enumerate(base_image['images']):
        img = img.resize((image_size, image_size))
        imglist.append(img)
        img_tensor = (PILToTensor()(img) / 255.0 - 0.5) * 2
        img_tensor = (PILToTensor()(img) / 255.0 - 0.5) * 2
        ft.append(dift.forward(img_tensor,
                        prompt=detail['subject'],
                        t=261,
                        ensemble_size=ensemble_size))
    ft = torch.cat(ft, dim=0)
    gc.collect()
    torch.cuda.empty_cache()
    image = consistory_pipe(prompt=prompts, generator=g_cpu, token_positions=detail['token_positions'], height=1024, width=1024, guidance_scale=5, dift_feature=ft)
