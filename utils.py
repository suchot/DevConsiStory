...
def mask_with_otsu_pytorch(tensor : torch.Tensor):
    ...
    # 二値化された画像を0~1のTensorに変換
    return (binary_image / 255.0).to(tensor.dtype)
...
# Function to recursively extract details
def extract_details(data, details_list, c_encode, c_decode):
    if isinstance(data, dict):
        if 'index' in data and 'concept_token' in data and 'prompts' in data:
            # Extract the position of concept_token in each prompt
            concept_token = data['concept_token']
            prompts = data['prompts']
            subject = data['subject']
            tokens = c_encode(prompts[0])
            token_positions = -1
            for idx, token in enumerate(tokens):
                if c_decode(int(token)) == data['concept_token']:
                    token_positions = idx 
            details_list.append({
                'index': data['index'],
        ...
        for key, value in data.items():
            extract_details(value, details_list, c_encode, c_decode)
    elif isinstance(data, list):
        for item in data:
            extract_details(item, details_list, c_encode, c_decode)
            
def concept_pprint(detail):
    print(f"Index: {detail['index']}")
    print(f"Concept Token: {detail['concept_token']}")
    print(f"Concept Token Positions: {detail['token_positions']}")
    print("Prompts:")
